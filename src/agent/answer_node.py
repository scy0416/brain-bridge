...
멀티턴 지원: state["question"](마지막 질문 하나)이 아니라
state["messages"](대화 히스토리 전체)를 build_answer_messages에 넘긴다.
Base/Router Agent는 여전히 question만 사용하지만(설계상 유지), Answer
Agent만 전체 히스토리를 참고해 자연스러운 대화 연속성을 낸다.
"""
...
def answer_agent_node(state: GraphState) -> dict:
    ...
    호출하지 않는다 (tools 파라미터 없이 순수 텍스트 생성만 수행).
    ...
    message = call_chat(answer_messages, tools=None, think=False)
